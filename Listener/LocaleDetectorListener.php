...
namespace Lunetics\LocaleBundle\Listener;
...
class LocaleDetectorListener
{
    ...
    public function onRequest(Event $event)
    {
        ...
        if ($session->has('localeIdentified')) {
            if (null !== $this->logger) {
                $this->logger->info(sprintf('Language Locale already Identified : [ %s ]', $session->getLocale()));
            }
            return;
        }
        ...
        if ($session->get('localeManually') !== true) {
            ...
            $session->set('localeIdentified', true);
            if (null !== $this->logger) {
